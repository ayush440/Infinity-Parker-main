...
if (config.env == "production") {
  ...
  baseUrl = ref("https://xvlg0zv7-8089.inc1.devtunnels.ms");
}
...
const endpoints: any = ref({
  ...
  purchasedCourses: '/user/invoice',
  courses: '/user/courses',
  chapters: '/user/courses/chapters',
  lessons: '/user/courses/chapters/lessons',
})
...
const getApiUrl = (endpoint: string) => {
  if (config.env != "production") {
    if (endpoint === "login" || endpoint === "logout" || endpoint === "profile" || endpoint === "updateUser" || endpoint === "changePassword" || endpoint === "register" || endpoint === "verifyOTP" || endpoint === "sendForgotOTP" || endpoint === "forgot") {
      baseUrl = ref("https://xvlg0zv7-8089.inc1.devtunnels.ms");
    } else if (endpoint === 'courses' || endpoint === 'chapters' || endpoint === 'lessons' || endpoint === 'purchasedCourses') {
      return `http://194.163.161.83:5002/digital${endpoints.value[endpoint]}`
    } else {
      baseUrl = ref("https://xvlg0zv7-8089.inc1.devtunnels.ms");
    }
  } else {
    if(endpoint === 'courses' || endpoint === 'chapters' || endpoint === 'lessons' || endpoint === 'purchasedCourses' ) {
      return `${currentRootUrl}/digital${endpoints.value[endpoint]}`;
    }
  }
...
const makeApiRequest = async (config: any, wait: number, endpoint: string, id: any) => {
  ...
  try {
    ...
    if (response.status === 200 && ((config.method === 'GET' && id && endpoint === 'chapters')|| endpoint === 'getBrokerOrders' || endpoint === 'login' || endpoint === 'sendForgotOTP' || endpoint === 'webhookAlerts' || (config.method === 'GET' && (!id || endpoint === 'webhookStrategyJoiners')))) {
