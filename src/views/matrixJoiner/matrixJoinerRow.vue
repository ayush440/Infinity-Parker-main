  ...
  <tr class="hidden sm:table-row intro-x">
    ...
    <td class="flex justify-center">
      <ButtonSwitch
        ...
        :id="`${matrixJoiner.id}is_active`"
        name="is_active"
        v-model="matrixJoiner.is_active"
      />
    </td>
    <td class="text-center text-nowrap">
      {{ formatDate(matrixJoiner.created_at) }}
    </td>
    <td class="">
  ...
  >
    <div :class="{ 'bg-secondary-dark font-semibold': fullDetails }">
      <div
        ...
        class="flex justify-between items-center py-3 text-[0.6rem] xs:text-xs cursor-pointer unselectable"
      >
        <!-- strategy -->
        <div class="flex flex-wrap items-center ml-1 min-w-[120px] uppercase">
          <template v-if="matrixJoiner.strategy">
            <component
              ...
              :style="{ color: matrixJoiner.strategy.color, strokeWidth: 2.5 }"
            />
            &nbsp;{{ getStrategyInitials(matrixJoiner.strategy.name) }}
            <div>({{ matrixJoiner.strategy.script }})</div>
          </template>
          <template v-else>_</template>
          <ArrowRight
            class="w-[12px] h-[12px] inline ml-1 transform rotate-90 transition transition-delay-300"
            :class="{ 'rotate-[-90deg]': fullDetails, 'rotate-90': !fullDetails }"
          />
        </div>
      ...
      >
        ...
        <div class="flex items-center">
          ...
          <div v-if="matrixJoiner.broker" class="flex ml-4">
            <img
              class="shadow-none w-3 h-3 mt-1"
              :src="images[matrixJoiner.broker.broker_name]"
            />
            &nbsp;{{ `${matrixJoiner.broker.broker_name}- ${matrixJoiner.broker.broker_userid}` }}
          </div>
        ...
        <div class="flex items-center">
          ...
          <div class="ml-4">{{ formatDate(matrixJoiner.created_at) }}</div>
        </div>
...
import { EditIcon, DeleteIcon, ArrowRight } from 'lucide-vue-next'
import ButtonSwitch from '../../../src/components/buttonSwitch.vue'
...
const props = defineProps({
  item: {
    type: Object as () => MatrixJoiner,
    required: true,
  },
})

const fullDetails = ref<boolean>(false)
const matrixJoiner = computed<MatrixJoiner>(() => props.item)

const handleActiveChange = async () => {
  try {
    const newState = !matrixJoiner.value.is_active;
    // Make API call first
    await addEditMatrixJoiner(matrixJoiner.value.id, { is_active: newState });
    // Only update local state if API call succeeds
    matrixJoiner.value.is_active = newState;
  } catch (error) {
    console.error('Failed to update active state:', error);
    // Optionally revert the UI state if API fails
    matrixJoiner.value.is_active = !matrixJoiner.value.is_active;
  }
}

const containsIcon = (data: MatrixJoiner) => {
  return data.strategy && data.strategy.image_url ? true : false
}

const showEdit = (data: MatrixJoiner) => {
...
const getStrategyInitials = (name: string) => {
  return name
    .split(' ')
    .map((word) => word[0])
    .join('')
